                    ...
                    <div class="col-12">
                        <div class="card">
                            <div class="card-header">
                                <h4>Liste des écoliers de {{ $maClasse }}</h4>
                            </div>

                            <div class="card-body">
                                {{-- Filtres --}}
                                <form action="{{ route('paiement.recherche') }}" method="GET">
                                <div class="row mb-4">
                                   
                                    @csrf
                                    <div class="col-md-5">
                                        <label for="filterClasse">Filtrer par classe</label>
                                        <select id="filterClasse" class="form-control" name="classe">
                                    ...
                                    <div class="col-md-5">
                                        ...
                                        <select id="filterAnnee" class="form-control" name="annee">
                                            <option value="">-- Toutes les années --</option>
                                            @foreach ($annees as $annee)
                                                <option value="{{ $annee->id }}"
                                                    {{ isset($derniereAnnee) && $annee->id == $derniereAnnee->id ? 'selected' : '' }}>
                                                    {{ $annee->annee }}
                                                </option>
                                            @endforeach
                                        </select>
                                        ...
                                        <tbody>
                                            @foreach ($inscriptions as $inscription)
                                                <tr>
                                                    ...
                                                    <td>{{ $inscription->anneScolaire->annee }}</td>
                                                    <td>
                                                        <a href="{{ route('inscription.show', ['inscription' => $inscription->id]) }}" class="btn-sm btn-info m-2" href="#">
                                                            <i class="fas fa-eye"></i> Détails
                                                        </a>
                                                        <button class="btn-sm btn-warning m-2" href="#" data-toggle="modal" data-target="#editEntry{{ $loop->iteration }}">
                                                            <i class="fas fa-edit"></i> Éditer
                                                        </button>
                                                        <form action="{{ route('inscription.delete', ['inscription' => $inscription->id]) }}" method="POST" style="display: inline;">
                                                            @csrf
                                                            @method('DELETE')
                                                            <button type="submit" class="btn-sm btn-danger m-2" onclick="return confirm('Êtes-vous sûr de vouloir supprimer cet élève ?')">
            ...
            <form action="{{ route('inscription.store') }}" method="POST">
                ...
                <div class="form-group">
                    ...
                    <select class="form-control" name="annee_id" required>
                        ...
                        <option value="{{ $derniereAnnee->id }}">{{ $derniereAnnee->annee }}</option>
                    </select>
                </div>
